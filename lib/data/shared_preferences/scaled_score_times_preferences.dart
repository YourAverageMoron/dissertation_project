...
class ScaledScoreTimesPreferences{
  ...
  Future<List<ScaledScoreTime>> getScaledScoreTimes() async{
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> stringTimes = prefs.getStringList(SCALED_SCORE_TIMES); //sort out the key
    //TODO TIDY THIS UP?
    return stringTimes.map((stringTime) =>
        ScaledScoreTime.fromJson(jsonDecode(stringTime))).toList();
  }
}